...
interface Seguro {
  id: number; seguradora: string; produto?: string; modalidade?: string; status: string;
  apolice?: string; segurado?: string; beneficiarios?: string; capital?: string;
  moeda: string; premio_mensal?: string; carencia_meses?: number;
  inicio?: string; vencimento?: string; seguradora_contato?: string; obs?: string;
  cobertura_morte?: boolean; cobertura_invalidez?: boolean; cobertura_doenca?: boolean;
  renovacao_auto?: boolean;
}

const KEY = "lifeInsurance_data";
...
export default function LifeInsurance() {
  ...
  function openForm(seg?: Seguro) {
    setForm(seg ? { ...seg } : { status: "ativo", moeda: "BRL", modalidade: "Temporário", cobertura_morte: true, carencia_meses: 6 });
    setOpen(true);
  }
  ...
  return (
    <AppLayout title={t.lifeInsurance}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold">{t.lifeInsurance}</h1>
          <p className="text-sm text-muted-foreground">{seguros.filter(s => s.status === "ativo").length} apólices ativas</p>
        </div>
        <Button onClick={() => openForm()}><Plus className="h-4 w-4 mr-1" />Nova Apólice</Button>
      </div>

      {seguros.length === 0 ? (
        <Card><CardContent className="p-8 text-center text-muted-foreground"><ShieldCheck className="h-10 w-10 mx-auto mb-2 opacity-40" /><p>Nenhuma apólice cadastrada.</p></CardContent></Card>
      ) : (
              ...
              <Card key={seg.id} className="hover:shadow-md transition-shadow">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div><div className="font-semibold">{seg.seguradora}</div><div className="text-xs text-muted-foreground">{seg.produto}</div></div>
                  ...
                  <div className="flex gap-2 mb-3"><Badge variant={st.variant}>{st.label}</Badge><Badge variant="outline">{seg.modalidade}</Badge><Badge variant="outline">{seg.moeda}</Badge></div>
                  <div className="text-xs text-muted-foreground space-y-1">
                    {seg.segurado && <div>Segurado: {seg.segurado}</div>}
                    {seg.capital && <div>Capital: {seg.moeda === "USD" ? "US$" : "R$"} {seg.capital}</div>}
                    {seg.premio_mensal && <div>Prêmio: {seg.moeda === "USD" ? "US$" : "R$"} {seg.premio_mensal}/mês</div>}
                    {(seg.inicio || seg.vencimento) && <div>{seg.inicio ?? "—"} → {seg.vencimento ?? "—"}</div>}
                  </div>
                </CardContent>
              </Card>
            );
        ...
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid gap-3 grid-cols-2">
            ...
            <div><Label>Modalidade</Label>
              <Select value={form.modalidade ?? "Temporário"} onValueChange={v => setForm(f => ({...f, modalidade: v}))}>
            ...
            <div><Label>Segurado</Label><Input value={form.segurado ?? ""} onChange={e => setForm(f => ({...f, segurado: e.target.value}))} /></div>
            <div><Label>Nº Apólice</Label><Input value={form.apolice ?? ""} onChange={e => setForm(f => ({...f, apolice: e.target.value}))} /></div>
            <div className="col-span-2"><Label>Beneficiários</Label><Input value={form.beneficiarios ?? ""} onChange={e => setForm(f => ({...f, beneficiarios: e.target.value}))} placeholder="Nome (%) — Nome (%)" /></div>
            <div><Label>Capital Segurado</Label><Input value={form.capital ?? ""} onChange={e => setForm(f => ({...f, capital: e.target.value}))} /></div>
            <div><Label>Moeda</Label>
              <Select value={form.moeda ?? "BRL"} onValueChange={v => setForm(f => ({...f, moeda: v}))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="BRL">BRL (R$)</SelectItem><SelectItem value="USD">USD (US$)</SelectItem></SelectContent>
              </Select>
            </div>
            <div><Label>Prêmio Mensal</Label><Input value={form.premio_mensal ?? ""} onChange={e => setForm(f => ({...f, premio_mensal: e.target.value}))} /></div>
            <div><Label>Carência (meses)</Label><Input type="number" value={form.carencia_meses ?? 6} onChange={e => setForm(f => ({...f, carencia_meses: Number(e.target.value)}))} /></div>
            ...
            <div className="col-span-2"><Label>Contato Seguradora</Label><Input value={form.seguradora_contato ?? ""} onChange={e => setForm(f => ({...f, seguradora_contato: e.target.value}))} /></div>
          </div>
