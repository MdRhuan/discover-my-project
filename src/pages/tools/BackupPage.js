...
function BackupPage() {
  ...
  async function importJSON(e) {
    ...
    try {
      ...
      await db.transaction('rw', db.empresas, db.funcionarios, db.documentos, db.transacoes, db.orgNodes, db.auditLog, async () => {
        await db.empresas.clear();     await db.empresas.bulkAdd(data.empresas||[]);
        await db.funcionarios.clear(); await db.funcionarios.bulkAdd(data.funcionarios||[]);
        await db.documentos.clear();   await db.documentos.bulkAdd(data.documentos||[]);
        await db.transacoes.clear();   await db.transacoes.bulkAdd(data.transacoes||[]);
        await db.orgNodes.clear();     await db.orgNodes.bulkAdd(data.orgNodes||[]);
      });
      await db.auditLog.add({ acao: `Backup importado: ${file.name}`, modulo: 'Backup', timestamp: new Date().toISOString() });
      toast(t.backupImported, 'success');
  ...
  async function resetAllData() {
    ...
    try {
      await db.transaction('rw',
        db.empresas, db.funcionarios, db.documentos, db.transacoes,
        db.orgNodes, db.orgTexts, db.tasks, db.alertas,
        db.docsPessoais, db.fiscalDocs, db.trademarks, db.config, db.auditLog,
        async () => {
          await Promise.all([
    ...
    <div className="page-content">
      <div className="page-header">
        <div className="page-header-info">
          <div className="page-header-title">{t.backup}</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,maxWidth:700}}>
        <div className="card" style={{textAlign:'center',padding:36}}>
          ...
          <div style={{fontFamily:'Playfair Display',fontWeight:700,fontSize:16,marginBottom:8}}>{t.exportJSON}</div>
        ...
        <div className="card" style={{textAlign:'center',padding:36}}>
          ...
          <div style={{fontFamily:'Playfair Display',fontWeight:700,fontSize:16,marginBottom:8}}>{t.importJSON}</div>
      ...
      <div className="card" style={{maxWidth:700,marginTop:24,border:'1px solid var(--red)',padding:24}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:8}}>
          ...
          <span style={{fontFamily:'Playfair Display',fontWeight:700,fontSize:15,color:'var(--red)'}}>Zona de Perigo</span>
        </div>
        ...
        <button
          className="btn"
          style={{background:'var(--red)',color:'#fff',border:'none',gap:8}}
      ...
      <div className="alert alert-warning" style={{marginTop:20,maxWidth:700}}>
        ...
        <div><strong>Dados armazenados localmente</strong> no IndexedDB do seu navegador. Faça backups regulares para evitar perda de dados ao limpar o histórico do browser.</div>
      </div>
      ...
      {confirmReset && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div className="card" style={{width:420,padding:32,textAlign:'center',boxShadow:'0 8px 40px rgba(0,0,0,.4)'}}>
            ...
            <div style={{fontFamily:'Playfair Display',fontWeight:700,fontSize:18,marginBottom:8}}>Tem certeza absoluta?</div>
            <p style={{color:'var(--text-muted)',fontSize:13,marginBottom:24}}>
              Todos os dados serão apagados permanentemente. Exporte um backup antes de continuar.
            </p>
            <div style={{display:'flex',gap:12,justifyContent:'center'}}>
              <button className="btn btn-ghost" style={{minWidth:120}} onClick={() => setConfirmReset(false)} disabled={resetting}>
                Cancelar
              </button>
              <button
                className="btn"
                style={{background:'var(--red)',color:'#fff',border:'none',minWidth:160}}
                onClick={resetAllData}
                disabled={resetting}
              >
                <i className={`fas ${resetting?'fa-spinner fa-spin':'fa-trash-can'}`}/>
