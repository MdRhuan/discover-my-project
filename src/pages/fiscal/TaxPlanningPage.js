...
function TaxPlanningPage() {
  ...
  const COLOR_VAR = { blue:'var(--blue)', green:'var(--green)' };
  const COLOR_BG  = { blue:'rgba(59,130,246,.12)', green:'rgba(34,197,94,.12)' };
  ...
  function openNew() {
    setForm({ subcategoria: filterSub || 'Tax Planning (US)', jurisdicao: filterSub === 'Tax Planning (BR)' ? 'BR' : 'US', tipo: 'PDF', status: 'ativo', ano: new Date().getFullYear().toString(), dataUpload: new Date().toISOString().slice(0,10) });
    setModal('form');
  }
    ...
    <div className="page-content">
      <div className="page-header">
        <div className="page-header-info">
          <div className="page-header-title">Tax Planning</div>
          <div className="page-header-sub">{rows.length} {t.taxPlanningSubtitle}</div>
        </div>
        ...
        {SUBCATS.map(s => {
          ...
          return (
            <button key={s.key} onClick={()=>setFilterSub(filterSub===s.key?'':s.key)}
              style={{display:'flex',alignItems:'center',gap:14,textAlign:'left',background:filterSub===s.key?'var(--surface-hover)':'var(--surface-card)',border:`1px solid ${filterSub===s.key?'var(--brand)':'var(--surface-border)'}`,borderRadius:12,padding:'14px 16px',cursor:'pointer'}}>
              <div style={{width:40,height:40,borderRadius:10,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',background:COLOR_BG[s.color]}}>
                <i className={`fas ${s.icon}`} style={{fontSize:16,color:COLOR_VAR[s.color]}}/>
              </div>
              <div>
                <div style={{fontWeight:700,color:'var(--text-primary)',fontSize:13}}>{s.label}</div>
      ...
      <div className="card" style={{marginBottom:16}}>
        <div style={{display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <select className="form-select" style={{maxWidth:200}} value={filterSub} onChange={e=>setFilterSub(e.target.value)}>
      ...
      <div className="card">
        {filtered.length === 0 ? (
          <div className="empty-state"><i className="fas fa-chess"/><p>{t.noDocumentsFound}</p></div>
        ) : (
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td>
                        {sub && <span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:11,color:COLOR_VAR[sub.color],fontWeight:600}}>
                          <i className={`fas ${sub.icon}`} style={{fontSize:10}}/>{r.subcategoria}
                        </span>}
                      </td>
                      <td>{r.ano||'—'}</td>
                      <td>{r.responsavel||'—'}</td>
        ...
        <div className="modal-backdrop" onClick={()=>setModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:560}}>
            <div className="modal-header">
              <div className="modal-title">{form.id ? t.editDocumentTax : t.newTaxPlanningDoc}</div>
            ...
            <div className="modal-body">
              <div className="form-grid">
                <div className="form-group" style={{gridColumn:'1/-1'}}>
                  ...
                  <input className="form-input" value={form.nome||''} onChange={e=>setForm(f=>({...f,nome:e.target.value}))} placeholder="Ex: Tax Planning Memo 2024"/>
                </div>
                <div className="form-group">
                  <label className="form-label">{t.categoryLabel}</label>
                  <select className="form-select" value={form.subcategoria||'Tax Planning (US)'} onChange={e=>setForm(f=>({...f,subcategoria:e.target.value,jurisdicao:e.target.value.includes('BR')?'BR':'US'}))}>
                    {SUBCATS.map(s=><option key={s.key} value={s.key}>{s.label}</option>)}
                  </select>
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.responsavel||''} onChange={e=>setForm(f=>({...f,responsavel:e.target.value}))} placeholder="Ex: Deloitte, Baker McKenzie"/>
                </div>
                <div className="form-group">
